...
function Sidebar({
  ...
  /** When collapsible="icon", temporarily reveal full content on hover without shifting layout. */
  hoverExpand?: boolean;
}) {
  const { isMobile, state, openMobile, setOpenMobile } = useSidebar();
  const [isHovering, setIsHovering] = React.useState(false);

  const canPeek = hoverExpand && collapsible === "icon" && !isMobile;
  const isPeeking = canPeek && state === "collapsed" && isHovering;
  ...
  // Gap sizing is computed directly from the REAL state, never from the
  // peeked/hover state — this is what keeps the page layout stable while
  // the sidebar visually expands over the content on hover.
  const gapWidthClass =
    state === "collapsed"
      ? collapsible === "icon"
  ...
  return (
    <div
      ...
      data-state={state}
      // Flips to "" while peeking, so every descendant relying on
      // group-data-[collapsible=icon] (hidden labels, icon-only buttons,
      // container width, ...) automatically reveals its expanded look —
      // no child component needs to know about hover state.
      data-collapsible={
        isPeeking ? "" : state === "collapsed" ? collapsible : ""
      }
      data-variant={variant}
      data-side={side}
      data-slot="sidebar"
    >
      {/* This is what handles the sidebar gap on desktop — sized from real state only */}
      <div
      ...
      <div
        ...
        onMouseEnter={() => canPeek && setIsHovering(true)}
        onMouseLeave={() => canPeek && setIsHovering(false)}
        className={cn(
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <MenuIcon className="rtl:rotate-180" />
